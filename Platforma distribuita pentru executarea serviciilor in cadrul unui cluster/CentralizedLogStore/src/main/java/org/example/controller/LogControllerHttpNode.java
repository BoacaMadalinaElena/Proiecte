...
import org.example.service.LogServiceHttpNode;
...
@RestController
@RequestMapping("/api/cluster/logsHttpNode")
@CrossOrigin
public class LogControllerHttpNode {
    @Autowired
    private LogServiceHttpNode logService;
    ...
    public ResponseEntity<?> insertInExecutionNodeLogs(@RequestHeader("Authorization") String authorizationHeader,
                                                       @RequestBody List<LogsMessageHttpNode> logs) {
        if(!authorizationHeader.equals(this.internPassword)){
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
        try {
            System.out.println("New /api/cluster/logsHttpNode post");
    ...
    public ResponseEntity<?> getAllByPage(@RequestHeader("Authorization") String authorizationHeader,
                                          ...
                                          @RequestParam(required = false,defaultValue = "") String typeNode) {
        ...
        try {
            ResponseGetLogsHttpNode logsMessageList = this.logService.getAll(page, size,time,address,type,typeNode);
