package org.example.sendLogs;

import com.opencsv.CSVReader;
import org.example.other.CustomPrinter;
import org.example.sendLogs.model.LogsMessage;
import java.io.File;
...
public class SendLogs implements Runnable {
    ...
    public void run() {
        ...
        String csvFile = "logs_master_" + newDateTime.format(DateTimeFormatter.ofPattern("dd_MM_yyyy_HH_mm")) + ".csv";
            ...
            if (file.exists()) {
                try {
                    ...
                    while ((nextRecord = csvReader.readNext()) != null) {
                        LogsMessage logsMessage = new LogsMessage(null, nextRecord[0], nextRecord[1], nextRecord[2], nextRecord[3]);
                        logsMessageList.add(logsMessage);
                    }

                    String url = "http://100.24.146.38:8070//api/cluster/logsMasterNode";
                    ...
                    if(file.delete())
                    {
                        CustomPrinter.printErr("Error delete file: " + file);
                    }
                } catch (Exception ex) {
                   CustomPrinter.printErr("Nu s-au putut trimite jurnalele!");
                }
            } else {
