...
-pentru verificare la cat se schimba un xor pe un bit la schimbare FU :) setat ca output
*/
...
//#define masurareImpulsFaraIntreruperi   //??????????nu  ERORI DESTUL DE MARI
...
//#define WD_timer_ms_101 //###__PB7__ frecventaRealaWatchDog

//#define WD_timer_hz_001 //###__PB10__frecventaRealaWatchDog
...
#define timpDeLaUltimulReset // timp ###

//#define integritateRAM_var1 

//#define integritateRAM_var2

//#define copiereDinFlashInRam
...
unsigned long long FOSC = 4016572;
...
void main()
{
  ...
  ICR1=FOSC/1000; // 4021 = valoarea calculata
...
void main( void )
{
  ...
  DDRD &= ~(1<<PD3);                    //pinul PD2 este de input
  
  TCCR1A=0;                             //initializare timer mod normal si dezactivare clock timer-ul nu numara 
  TCCR1B=0;
  
  while((PIND & (1<<PD3)) == (1<<PD3)) //asteptam frontul negativ
  {}
  while((PIND & (1<<PD3))==0); //asteptam frontul pozitiv
  {
  ...
  while((PIND & (1<<PD3))==(1<<PD3))   //asteptam frontul negativ
  {}
  while((PIND & (1<<PD3))==0);          //asteptam frontul pozitiv pentru a termina de masurat
  {
    //a aparut frontul pozitiv
  ...
  nr_us=(nrOvf *65536 + nr_cicli) * 248968/ 1000000  ; //nr_us=nrciclii* (perioada unui ciclu, adica 0.25 microsecunde) * prescaler(1)
...
{ 
  ...
  while(1)
  {
      ...
      // daca flag este nenul, inseamna ca am avut front prinul numar transmis nu se pune :)
    {
      ...
      frecventa = 1000000000/period;    //in mHz
...
unsigned long long int FOSC = 4016572;
#pragma vector=USART_RXC_vect
__interrupt void usart_interrupt(void)
{
  switch(UDR)
  ...
  case '+':
    ...
    ICR1=(unsigned int)(FOSC/countHz);             //noua frecventa
...
void main()
{
  ...
  timer1_INIT();
  //Valoarea care se incarca in registru in functie de frecventa necesara
  ICR1=(unsigned int)(FOSC/countHz);
...
__no_init unsigned int nrAfisari;
...
void main( void )
{
  ...
  timePass *= 65536;
  timePass += timerVal;
  timePass = timePass * 248968 / 1000000; //perioada time-out in us
...
int main()
{
  ...
  unsigned int crc16_lent = crc16__flash(CRC16_CCITT,0,(unsigned int)nrXXXX,nrOcteti,MSBF);
  unsigned int crc16_rapid = crc16wtable__flash(0,(unsigned int)nrXXXX,nrOcteti);
...
__interrupt void T1_INT(void){
  cnt++;
  if(cnt > 50000){//asta ar trebui sa masoare o secnda
    flag = 1;
    cnt = 0;
  }
}
...
void main( void )
{
  ...
  unsigned long long freq = 32768;
  freq = freq * 1000000000 / timePass;//in mHz
...
unsigned long long FOSC = 4004841;
...
#ifdef integritateRAM_var1
/*
1. Însusirea de a fi integru.
2. Integritate teritorialã = principiu de bazã al dreptului interna?ional 
potrivit cãruia fiecare stat are dreptul sã-?i exercite deplin ?i nestingherit 
suveranitatea asupra teritoriului sãu.
https://dexonline.ro/definitie/integritate
*/

unsigned int strlen(unsigned char sir[])
{
  unsigned int nr = 0;
  while(sir[nr])
  {
...
void f(void)
{
  unsigned char sir1[] = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x0};
}

int main()
{
  unsigned char sir1[] = {0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0x0};
  unsigned char sir2[] = {0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x0};
  unsigned int size1 = strlen(sir1);
  unsigned int size2 = strlen(sir2);
  unsigned int addrStart1 = (unsigned int )sir1;
  unsigned int addrStart2 = (unsigned int )sir2;
  unsigned int addrStop1 = addrStart1 + size1;
  unsigned int addrStop2 = addrStart1 + size2;
  sir1[0] = 0x21;
  sir1[1] = 0x22;
  sir1[2] = 0x23;
  sir1[3] = 0x24;
  ...
  f();
  //schimb sir 2 sir 1 ramane la fel 
  return 0;
}
#endif
...
int main()
{
  ...
  unsigned int crc1_v1 = crc16wtable__data(0,addrStart1,size1);
  unsigned int crc2_v1 = crc16wtable__data(0,addrStart2,size2);
  ...
  unsigned int crc1_v2 = crc16wtable__data(0,addrStart1,size1);
  unsigned int crc2_v2 = crc16wtable__data(0,addrStart2,size2);
  
  f();
  
  unsigned int crc1_v3 = crc16wtable__data(0,addrStart1,size1);
  unsigned int crc2_v3 = crc16wtable__data(0,addrStart2,size2);
...
#ifdef copiereDinFlashInRam

int main()
{
  unsigned char sir[10];
  unsigned int i ;
  for(i = 0;i<10;i++)
  {
    sir[i] = *(__flash unsigned char *)(i);
  }
  //0c 94 42 00 18 95 18 95 18 95
  return 0;
}
