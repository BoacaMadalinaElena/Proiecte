...
unsigned int crc16wtable__flash(unsigned int init_val_16,unsigned int adr_start, unsigned int len) 
{ 
...
unsigned int crc16__data(unsigned int polinom16, unsigned int init_val_16, unsigned int adr_start,unsigned int len, enum BitOrder ord)
{
  // rezultatul final
  unsigned int crc = init_val_16;
  
  // retine in octetul cel mai semnificativ datele extrase din memoria ram
  unsigned int data = 0;
  while(len--) 
  {
    unsigned int i;
    
    // se extrage valoarea unui octet de la adresa de start din memoria ram
    data = *(unsigned char *)adr_start;
    ...
    if (ord == MSBF) // optiunea cu shiftare spre MSB
    {
      ...
      for( i = 0; i < 8; ++i ) 
      {
        // se verifica dac? bitul cel mai semnificativ este 1
        if( crc & 0x8000 )
          crc = (crc << 1) ^ polinom16;
        else
          crc = crc << 1;
    ...
    else // optiunea cu shiftare spre LSB
    {
      ...
      // se verific? bitul cel mai putin semnificativ si daca acesta este 1 se face XOR cu polinomul generator, altfel datele sunt shiftate la dreapta
      for( i = 0; i < 8; ++i ) 
      {
...
unsigned int crc16wtable__data(unsigned int init_val_16,unsigned int adr_start, unsigned int len) 
{  
  unsigned int crc = init_val_16; 
  for(unsigned int counter = 0; counter < len; counter++) 
    crc = (crc<<8) ^ crc16tab[((crc>>8) ^ *(unsigned char *)adr_start++) & 0x00FF]; 
  return crc; 
}
